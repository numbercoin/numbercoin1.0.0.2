...
const string strMessageMagic = "NumberCoin Signed Message:\n";
...
int64 GetProofOfWorkReward(int nHeight, int64 nFees, uint256 prevHash)
{
  ...
  else if(nHeight >= 2 && nHeight <= 100 )
  {
    ...
    else if(nHeight > 100 && nHeight < 4320)
	{
        nSubsidy = 200 * COIN;
		return nSubsidy + nFees;
	}
...
int64 GetProofOfStakeReward(int64 nCoinAge, unsigned int nBits, unsigned int nTime, int nHeight)
{
	...
	if(pindexBest->nHeight < (15 * DAILY_BLOCKCOUNT))
        ...
        nRewardCoinYear = 750 * MAX_MINT_PROOF_OF_STAKE;
    else if(pindexBest->nHeight < (150 * DAILY_BLOCKCOUNT))
        nRewardCoinYear = 1 * MAX_MINT_PROOF_OF_STAKE;
    else if(pindexBest->nHeight < (160 * DAILY_BLOCKCOUNT))
        nRewardCoinYear = 325 * MAX_MINT_PROOF_OF_STAKE;
    else if(pindexBest->nHeight < (365 * DAILY_BLOCKCOUNT))
        nRewardCoinYear = 1 * MAX_MINT_PROOF_OF_STAKE;
    else if(pindexBest->nHeight < (380 * DAILY_BLOCKCOUNT))
        nRewardCoinYear = 162.5 * MAX_MINT_PROOF_OF_STAKE;
    else if(pindexBest->nHeight < (730 * DAILY_BLOCKCOUNT))
        nRewardCoinYear = 1 * MAX_MINT_PROOF_OF_STAKE;
    else if(pindexBest->nHeight < (750 * DAILY_BLOCKCOUNT))
        nRewardCoinYear = 81.25 * MAX_MINT_PROOF_OF_STAKE;
    else if(pindexBest->nHeight < (1095 * DAILY_BLOCKCOUNT))
        nRewardCoinYear = 1 * MAX_MINT_PROOF_OF_STAKE;
    else if(pindexBest->nHeight < (1195 * DAILY_BLOCKCOUNT))
        nRewardCoinYear = 40.5125 * MAX_MINT_PROOF_OF_STAKE;
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in NumberCoin
    bool fStrictPayToScriptHash = true; // Always active in NumberCoin
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "NumberCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "good NumberCoin .";
        CTransaction txNew;
        ...
        assert(block.hashMerkleRoot == uint256("0xb9b0ce3dae5ee5d51fc601b053e06846cc09aebcf5acacff639ce72c7b2de61b"));
